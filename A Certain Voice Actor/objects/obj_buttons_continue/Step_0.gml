...
if menu = 1 {
	...
	if(selected = 1 and global.selectbuttonpressed){
		file_delete("A_Certain_Voice_Actor.json")
		game_restart()
	}
...
if menu = 3{
	if timer <= 0 {
		if selected = 0 and global.selectbuttonpressed{
			...
			menu = 4
			selected = 0
			letter = obj_initialize.selectkey
			timer2 = 1
		}
	}
	timer--
	if selected = 1 and global.selectbuttonpressed{
		...
		menu = 4
		selected = 0
		letter = obj_initialize.cancelkey
		timer2 = 1
	}
	if selected = 2 and global.selectbuttonpressed{
		control = 2
	...
	if selected = 6 and global.selectbuttonpressed{
		...
		menu = 4
		selected = 0
		letter = obj_initialize.rightkey
		timer2 = 1
	}
	//cancel
	if keyboard_check_pressed(global.cancelbutton){
